...
static void test_visitor_out_intList(TestOutputVisitorData *data,
                                     const void *unused)
{
    int64_t value[] = {-10, -7, -2, -1, 0, 1, 9, 10, 16, 15, 14, 3, 4, 5, 6, 11, 12, 13, 21, 22, INT64_MAX - 1, INT64_MAX};
    ...
    str = string_output_get_string(data->sov);
    g_assert(str != NULL);
    g_assert_cmpstr(str, ==, "-10,-7,-2-1,3-6,9-16,21-22,9223372036854775806-9223372036854775807");
    g_free(str);
#if 0
    while (list) {
        tmp2 = list->next;
